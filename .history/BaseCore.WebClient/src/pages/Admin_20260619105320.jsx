...
function BusesManager({ operators: initialOperators = [], onRefresh, isOperator = false, refreshKey = 0 }) {
  ...
  const [urlInput, setUrlInput] = useState('');
   const [urlIsAvatar, setUrlIsAvatar] = useState(false);
  const [page, setPage] = useState(1);
  const [loading, setLoading] = useState(false);
  const [notice, setNotice] = useState(null);
  ...
  // THAY closeImgModal thành:
const closeImgModal = () => {
  setUrlInput('');
  setUrlIsAvatar(false);
  setImgModal(null);
};
...
const saveImageUrl = async () => {
  ...
  setImgLoading(true);
  try {
    const result = await busApi.addImageUrl(imgModal.busId, urlInput.trim());
    if (urlIsAvatar) {
      const imageId = result?.imageID ?? result?.ImageID;
      if (imageId) await busApi.setAvatar(imageId);
    }
    setUrlInput('');
    setUrlIsAvatar(false);
    const imgs = await busApi.getImages(imgModal.busId);
      ...
      {imgModal && createPortal(
        <div className="modal-overlay" onClick={closeImgModal}>
          <div className="modal-box bus-img-modal" onClick={(e) => e.stopPropagation()}>
            ...
            {/* Thêm ảnh bằng URL */}
          <div className="bus-img-add-row" style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap', padding: '12px 0' }}>
            <input
              type="url"
              value={urlInput}
            ...
            <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, whiteSpace: 'nowrap', cursor: 'pointer' }}>
              <input
                type="checkbox"
                checked={urlIsAvatar}
                onChange={(e) => setUrlIsAvatar(e.target.checked)}
              />
              Đặt làm đại diện
            </label>
            <button
            ...
            >
              <i className="fa-solid fa-plus" /> Lưu ảnh
            </button>
          </div>
