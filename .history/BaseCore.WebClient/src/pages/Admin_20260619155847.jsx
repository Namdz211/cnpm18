    ...
    <section className="admin-card table-card">
      ...
      <div className="admin-filter-grid trips-filter-grid">
        ...
        <input
          ...
          style={{ display: 'none', minWidth: 110 }}
        />
        <input
          type="number"
          min="0"
...
function PromotionsManager() {
  ...
  const [filterFrom, setFilterFrom] = useState("");       // ngày bắt đầu lọc
  const [filterTo, setFilterTo] = useState("");           // ngày kết thúc lọc
  ...
  return (
    <section className="admin-card table-card">
      ...
      {/* Filter bar */}
      <div className="admin-filter-grid" style={{ alignItems: "center" }}>
        <select value={filterType} onChange={(e) => setFilterType(e.target.value)}>
          <option value="">Tất cả loại</option>
          <option value="1">Phần trăm (%)</option>
          <option value="2">Cố định (đ)</option>
        </select>
        <label className={`admin-date-input ${filterFrom ? "has-value" : ""}`} style={{display : "none"}}>
          <span>Từ ngày</span>
          <strong>{filterFrom ? new Date(filterFrom).toLocaleDateString("vi-VN") : "Chọn ngày"}</strong>
          <i className="fa-regular fa-calendar-days" />
          <input
            type="date"
            value={filterFrom}
        ...
        <label className={`admin-date-input ${filterTo ? "has-value" : ""}`} style={{display : "none"}}>
          <span>Đến ngày</span>
          <strong>{filterTo ? new Date(filterTo).toLocaleDateString("vi-VN") : "Chọn ngày"}</strong>
          <i className="fa-regular fa-calendar-days" />
          <input
            type="date"
            value={filterTo}
      ...
      <div className="table-wrap">
        <table>
          ...
          <tbody>
            {rows
              .filter((item) => {
                if (filterType && String(pick(item, ["discountType", "DiscountType"], 1)) !== filterType) return false;
                const start = dateOnly(pick(item, ["startDate", "StartDate"]));
                const end   = dateOnly(pick(item, ["endDate",   "EndDate"]));
            ...
            {!loading && rows.length === 0 && (
              <tr>
                <td colSpan="10" className="empty-cell">Chưa có mã giảm giá.</td>
              </tr>
            )}
            {!loading && rows.length > 0 && rows.filter((item) => {
              if (filterType && String(pick(item, ["discountType", "DiscountType"], 1)) !== filterType) return false;
              const start = dateOnly(pick(item, ["startDate", "StartDate"]));
              const end   = dateOnly(pick(item, ["endDate",   "EndDate"]));
              if (filterFrom && end   < filterFrom) return false;
              ...
              <tr>
                <td colSpan="10" className="empty-cell">Không có mã nào khớp với bộ lọc.</td>
              </tr>
            )}
          </tbody>
        </table>
...
function BusesManager({ operators: initialOperators = [], onRefresh, isOperator = false, refreshKey = 0 }) {
  ...
  // ▼▼▼ CHUYỂN CHẾ ĐỘ ẢNH: chỉ dùng 1 trong 2 khối dưới ▼▼▼
  // === URL MODE (đang bật) — comment 1 dòng dưới + bỏ comment UPLOAD để đổi ===
  const [urlInput, setUrlInput] = useState("");
  // === UPLOAD MODE — bỏ comment 3 dòng dưới + comment dòng urlInput trên để đổi ===
  // const [pendingFiles, setPendingFiles] = useState([]);
  // const [pendingAvatar, setPendingAvatar] = useState(null);
  // const fileInputRef = useRef(null);
  // ▲▲▲ END CHUYỂN CHẾ ĐỘ ẢNH ▲▲▲
  const [page, setPage] = useState(1);
  const [loading, setLoading] = useState(false);
  const [notice, setNotice] = useState(null);
  ...
  const closeImgModal = () => {
    // === URL MODE (đang bật) — không cần dọn file ===
    // === UPLOAD MODE — bỏ comment 3 dòng dưới để đổi ===
  ...
  // === UPLOAD MODE — bỏ comment toàn bộ khối dưới để đổi sang upload ===
  ...
  // === URL MODE (đang bật) — comment hàm saveUrl dưới khi đổi sang upload ===
  const saveUrl = async () => {
    if (!urlInput.trim() || !imgModal) return;
    setImgLoading(true);
    try {
      await busApi.addImage({ busID: imgModal.busId, imageURL: urlInput.trim(), isAvatar: false, sortOrder: 0 });
      setUrlInput("");
      const imgs = await busApi.getImages(imgModal.busId);
      setImgModal((m) => ({ ...m, images: Array.isArray(imgs) ? imgs : [] }));
    } catch (e) {
      alert(e.message || "Không thêm được ảnh.");
    } finally {
      setImgLoading(false);
    }
  };
      ...
      {imgModal && createPortal(
        <div className="modal-overlay" onClick={closeImgModal}>
          <div className="modal-box bus-img-modal" onClick={(e) => e.stopPropagation()}>
            ...
            {/* ▼ URL MODE (đang bật) — comment khối này + bỏ comment UPLOAD bên dưới để đổi ▼ */}
            <div className="bus-img-add-row">
              ...
              <button
                className="btn btn-primary"
                onClick={saveUrl}
                disabled={imgLoading || !urlInput.trim()}
              >
            ...
            {/* ▼ UPLOAD MODE — bỏ comment khối này + comment URL trên để đổi ▼ */}
            {/* <div className="bus-img-add-row">
              <input
                ref={fileInputRef}
                type="file"
            ...
            {/* URL MODE: chỉ check images; UPLOAD MODE: thêm && pendingFiles.length === 0 vào điều kiện và đổi text */}
            {!imgLoading && imgModal.images.length === 0 && (
              <div className="bus-img-empty">Chưa có ảnh nào. Nhập URL ảnh và nhấn "Thêm ảnh".</div>
            )}
            ...
            {/* ▼ UPLOAD MODE — bỏ comment khối "Ảnh chờ lưu" dưới để đổi ▼ */}
            ...
            <div className="bus-img-footer">
              ...
              {/* ▼ UPLOAD MODE — bỏ comment nút Lưu dưới để đổi ▼ */}
              {/* <button
                className="btn btn-primary"
                onClick={saveImages}
...
function TripsTable({ trips, onEdit, onDelete, onCancel, onRowClick }) {
  ...
  const sortedTrips = sortDir
    ? [...trips].sort((a, b) => {
        const va = a.availableSeats * a.price;
        const vb = b.availableSeats * b.price;
        ...
        <thead>
          <tr>
            ...
            <th onClick={toggleSort} style={{ cursor: 'pointer', userSelect: 'none' }}>
              Thành Tiền {sortDir === 'asc' ? '↑' : sortDir === 'desc' ? '↓' : '↕'}
            </th>
            <th>Giá</th>
          ...
          {sortedTrips.map((t) => {
            ...
            return (
              <tr key={t.id}>
                ...
                <td>{formatVND(t.availableSeats * t.price)}</td>
